...
// Improved team name cleaning for ESPN
function cleanTeams(text) {
  let t = text.replace(/\s+/g, ' ').trim();
  
  // Remove common ESPN prefixes and suffixes
  t = t.replace(/^FC\s+/i, '')
       .replace(/\s+FC$/i, '')
       .replace(/^AFC\s+/i, '')
       .replace(/\s+Women$/i, '')
       .replace(/\s+\(\w+\)$/i, ''); // Remove country codes in parentheses
  ...
  for (const separator of separators) {
    ...
    if (parts.length === 2) {
      ...
      home = home.replace(/\(\w+\)$/, '').trim();
      away = away.replace(/\(\w+\)$/, '').trim();
...
async function sourceEspnScheduleToday(tz = TZ){
  ...
  if (ESPN_DEBUG) {
    console.log('[ESPN] URL:', url, 'tables:', tables.length, 'rows:', rows.length, 'unknown:', unknown);
