...
async function sourceEspnScheduleToday(tz = TZ){
  ...
  tables.each((_, tbl)=>{
    ...
    if (!isWantedLeague(league)) {
      if (ESPN_DEBUG) console.log('[ESPN] Filtered out league:', league);
      filteredLeagues++;
      return;
    }
    ...
    $tbl.find('tbody tr').each((__, tr)=>{
      ...
      if (ESPN_DEBUG) console.log('[ESPN] Successfully parsed:', { league, original: matchText, home, away });
      successfulParses++;
  ...
  if (ESPN_DEBUG) {
    console.log('[ESPN] URL:', url, 'tables:', tables.length, 'successful parses:', successfulParses, 'rows:', rows.length, 'filtered leagues:', filteredLeagues);
    if (rows.length > 0) {
      ...
      console.log('[ESPN] No matches found for today in major leagues');
    }
  }

  return { rows, meta:{ name:'espn', used:true, count: rows.length, successfulParses, filteredLeagues, url, unknown } };
}
...
app.get('/diag-espn', async (_req, res)=>{
  ...
  res.json({ url: out.meta?.url, count: out.meta?.count||0, successfulParses: out.meta?.successfulParses||0, filteredLeagues: out.meta?.filteredLeagues||0, unknown: out.meta?.unknown||0, sample: (out.rows||[]).slice(0,8) });
});
